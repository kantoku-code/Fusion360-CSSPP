...
class Fusion360CommandBase:
    ...
    def on_validateiputs(self, command: adsk.core.Command, inputs: adsk.core.CommandInputs, args, input_values):
        pass
...
class CommandCreatedEventHandler(adsk.core.CommandCreatedEventHandler):
    ...
    def notify(self, args):
        ...
        try:
            ...
            on_validateinputs_handler = ValidateInputsHandler(self.cmd_object_)
            command_.validateInputs.add(on_validateinputs_handler)
            handlers.append(on_validateinputs_handler)
...
class ValidateInputsHandler(adsk.core.ValidateInputsEventHandler):
    ...
    def notify(self, args):
        ...
        try:
            ...
            if self.cmd_object_.debug:
                ui.messageBox('***Debug command: {} validateInputs successfully'.format(command_.parentCommandDefinition.id))

            input_values = get_inputs(command_inputs)

            self.cmd_object_.on_validateInputs(command_, command_inputs, args, input_values)

        except:
            if ui:
                ui.messageBox('command validateInputs failed: {}'.format(traceback.format_exc()))
